...
class SinaSource(AbstractSource):
    ...
    def _fetch_index_batch(self, symbols: list[str]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            raise SourceError(f"sina index quote failed: {exc}") from exc
        return self._parse_index_response(resp.text, set(symbols))

    def _parse_index_response(
        self, text: str, symbol_set: set[str]
    ) -> list[dict[str, Any]]:
        """Parse Sina response for index symbols; code is the full symbol e.g. 'sh000001'."""
        results = []
